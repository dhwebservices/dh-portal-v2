...
export async function buildContractPdfBlob(contract = {}) {
  ...
  try {
    ...
    if (!canvas?.width || !canvas?.height) {
      throw new Error('Could not render the contract into a signed PDF.')
    }
    ...
    if (!Number.isFinite(imgHeight) || imgHeight <= 0) {
      throw new Error('Could not calculate the signed contract PDF page size.')
    }
    ...
    if (!ctx) {
      throw new Error('Could not prepare the signed contract PDF canvas.')
    }
    ...
    while (remainingHeight > 0 && sourceY < canvas.height) {
      ...
      if (!Number.isFinite(renderedHeight) || renderedHeight <= 0) {
        throw new Error('Could not render a page of the signed contract PDF.')
      }
