...
const PERMISSION_GROUPS = ['Home', 'Outreach', 'My Work', 'People', 'Recruiting', 'Websites', 'Support', 'Reports', 'Admin']
...
function getPermissionItemsForGroup(group) {
  if (group === 'Recruiting') return [HIRING_WORKSPACE_PERMISSION]
...
function getPermissionEnabledCount(perms = {}, group) {
  if (group === 'Recruiting') return isHiringWorkspaceEnabled(perms) ? 1 : 0
  ...
  const save = async () => {
    ...
    try {
      ...
      if (newMgr && newMgr !== prevMgr) {
        ...
        try {
          await sendEmail('manager_assigned', {
            to_email: newMgr,
            manager_name: mgr,
        ...
        try {
          await sendEmail('staff_manager_assigned', {
            to_email: email,
            staff_name: staffName,
            manager_name: mgr,
            manager_email: newMgr,
            assigned_by: user?.name || 'Admin',
          })
        } catch (_) {}
        setPrevMgr(newMgr)
                  ...
                  <div style={{ padding:'12px 14px', borderBottom:'1px solid var(--border)', display:'flex', justifyContent:'space-between', gap:12, alignItems:'center', flexWrap:'wrap' }}>
                    ...
                    <div style={{ display:'flex', gap:8 }}>
                      <button className="btn btn-outline btn-sm" onClick={() => setEditPerms((current) => {
                        if (group === 'Recruiting') return setHiringWorkspacePermission(current, true)
                        const next = { ...current }
                        items.forEach(({ key }) => { next[key] = true })
                        return next
                      })}>Enable all</button>
                      <button className="btn btn-outline btn-sm" onClick={() => setEditPerms((current) => {
                        if (group === 'Recruiting') return setHiringWorkspacePermission(current, false)
                        const next = { ...current }
